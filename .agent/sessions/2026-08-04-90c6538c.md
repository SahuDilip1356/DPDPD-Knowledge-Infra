...
## 2026-08-04 21:22 — mid-session checkpoint
...
**Last assistant note:**
[REDACTED — secret-like content removed by mem_capture]
   plus `ALLOWED_ORIGINS` = `https://dpdpa.wiki,https://dpdpa.shiksha`
   **Do not set `PORT` or `DATABASE_URL`** — Railway injects the first, and blank is correct for the second.
4. **Settings → Networking → Generate Domain**
5. Check `https://your-url.up.railway.app/health` returns `{"status":"HEALTHY"}` and send me the URL

Then I'll wire `VITE_API_URL` into both frontends and you can create the two Vercel projects — shiksha imports its repo directly with no root directory, wiki uses `deployments/dpdpa-wiki`.
